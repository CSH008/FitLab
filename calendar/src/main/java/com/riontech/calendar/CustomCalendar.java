...
public class CustomCalendar extends LinearLayout implements ViewPager.OnPageChangeListener {
    ...
    private void initViews() {
        ...
        mTxtEventMessage.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View view) {
                try {
                    Date dateTemp = CalendarUtils.getCalendarDBFormat().parse(Singleton.getInstance().getCurrentDate());
                    mAddEventListener.onTapToAddClick(CalendarUtils.getCalendarDateFormat().format(dateTemp));
...
//        });
    ...
    public void nextMonth() {
        if(mViewPager.getCurrentItem() < mTotalMonthCount){
    ...
    public void refreshCalendar(String date) {
        // Log.e(TAG, "date: " + date);
        CalendarFragment.fetchEvents(date);
    ...
    public void setDateSelectionData(ArrayList<EventData> dateData) {
        ...
        if (items.size() == 0) {
            ...
            try {
                Date dateTemp = CalendarUtils.getCalendarDBFormat().parse(Singleton.getInstance().getCurrentDate());
                mTxtEventMessage.setText(getResources().getString(R.string.no_events) + " (" + CalendarUtils.getCalendarDateFormat().format(dateTemp) + "). " + getResources().getString(R.string.tap_to_add));
            ...
            mRvCalendar.setLayoutManager(mLinearLayoutManager);
            mRvCalendar.setAdapter(new CalendarDataAdapter(mAddEventListener, items));
        }
    }
    ...
    public void initListener(addEventListener calendarFragment) {
        mAddEventListener = calendarFragment;
    }
