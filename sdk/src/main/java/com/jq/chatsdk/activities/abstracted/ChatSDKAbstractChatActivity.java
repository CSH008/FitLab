...
package com.jq.chatsdk.activities.abstracted;
...
import com.jq.chatsdk.Utils.NotificationUtils;
import com.jq.chatsdk.Utils.asynctask.MakeThreadImage;
import com.jq.chatsdk.Utils.helper.ChatSDKChatHelper;
import com.jq.chatsdk.Utils.volley.VolleyUtils;
import com.jq.chatsdk.activities.ChatSDKBaseActivity;
import com.jq.chatsdk.activities.ChatSDKBaseThreadActivity;
...
import com.jq.chatsdk.dao.BThreadDao;
...
import com.jq.chatsdk.network.events.BatchedEvent;
import com.jq.chatsdk.network.events.Event;
import com.jq.chatsdk.network.events.MessageEventListener;
import com.jq.chatsdk.object.Batcher;
import com.jq.chatsdk.thread.ChatSDKImageMessagesThreadPool;
...
//                if (thread.getType()==BThread.Type.Private)
    ...
    protected void initActionBar() {
        ...
        setThreadName(txtName);

        View chat_sdk_image_container = findViewById(R.id.chat_sdk_image_container);
        chat_sdk_image_container.setVisibility(View.VISIBLE);
        final CircleImageView circleImageView = findViewById(R.id.chat_sdk_circle_image);
        final ImageView imageView = findViewById(R.id.chat_sdk_round_corner_image);
        ...
        setThreadImage(circleImageView, imageView, onClickListener);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        } else if(requestCode == ChooserType.REQUEST_CAPTURE_VIDEO) {
            if (chatSDKChatHelper.videoChooserManager == null) {
                chatSDKChatHelper.reinitializeVideoChooser();
            }
            chatSDKChatHelper.videoChooserManager.submit(requestCode, data);

        } else if(requestCode == ChooserType.REQUEST_PICK_VIDEO) {
            if (chatSDKChatHelper.videoChooserManager == null) {
                chatSDKChatHelper.reinitializeVideoChooser();
            }
            chatSDKChatHelper.videoChooserManager.submit(requestCode, data);
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        if (BDefines.Options.ThreadDetailsEnabled)
        {
            ...
            itemThreadUsers.setIcon(R.drawable.ic_info);
        }
