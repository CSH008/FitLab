...
package com.jq.chatsdk.network;
...
import com.jq.chatsdk.Utils.sorter.ThreadsItemSorter;
import com.jq.chatsdk.Utils.sorter.ThreadsSorter;
import com.jq.chatsdk.Utils.volley.VolleyUtils;
import com.jq.chatsdk.adapter.abstracted.ChatSDKAbstractThreadsListAdapter;
...
import com.jq.chatsdk.network.events.AbstractEventManager;
import com.jq.chatsdk.object.BError;
import com.jq.chatsdk.object.SaveImageProgress;
...
import static com.jq.chatsdk.dao.entities.BMessageEntity.Type.IMAGE;
import static com.jq.chatsdk.dao.entities.BMessageEntity.Type.LOCATION;
import static com.jq.chatsdk.dao.entities.BMessageEntity.Type.TEXT;
import static com.jq.chatsdk.dao.entities.BMessageEntity.Type.VIDEO;
import static com.jq.chatsdk.network.BDefines.Prefs.AuthenticationID;
...
public abstract class AbstractNetworkAdapter {
    ...
    public Promise<BMessage, BError, BMessage>  sendMessageWithImage(final String filePath, long threadId) {
        ...
        if(image!=null) {
            ...
            message.setImageDimensions("W100&H100");
        }
    ...
    public Promise<BMessage, BError, BMessage>  sendMessageWithVideo(final String filePath, long threadId) {
        ...
        uploadVideo(new File(filePath), thumbnail)
                .progress(new ProgressCallback<SaveImageProgress>() {
                    @Override
                    public void onProgress(SaveImageProgress saveImageProgress) {
                        deferred.notify(message);
...
//                deferred.notify(message);
    ...
    public Promise<String[], BError, SaveImageProgress> uploadVideo(final File video, final Bitmap thumbnail) {
        ...
        byte[] videoData = FileUtils.getByteArrayFromFile(video);
        if(videoData.length<3) {
            deferred.reject(new BError(BError.Code.FILE_EXCEPTION, "File exception."));
        }

        uploadHandler.uploadFile(FileUtils.getByteArrayFromFile(video), "video.mp4", "video/mp4")
                .done(new DoneCallback<String>() {
                    @Override
                    public void onDone(String url) {
                        urls[0] = url;
